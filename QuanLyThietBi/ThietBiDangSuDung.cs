...
{
    public partial class ThietBiDangSuDung : Form
    {
        BindingSource ThietBiSuDungList = new BindingSource();

        public ThietBiDangSuDung()
        {
            InitializeComponent();
            ...
            dgTBsudung.Columns["Maloaithietbi"].Visible = false;
        }

        private void btnThemTB_Click(object sender, EventArgs e)
        {
            ThemThietBiDangSuDung form = new ThemThietBiDangSuDung();
            this.Hide();
            form.ShowDialog();
            this.Show();
            LoadDanhSachThietBiSuDung();
        ...
        {
            txtTenthietbi.DataBindings.Add(new Binding("Text", dgTBsudung.DataSource, "Maloaithietbi", true, DataSourceUpdateMode.Never));
